...
resource "local_file" "ansible_inventory" {
  content = templatefile("${path.module}/../ansible/inventory.yaml.tpl", {
    ...
    worker_name_prefix= "${local.CLUSTER_NAME}_worker"
    control_plane_name_prefix= "${local.CLUSTER_NAME}_control_plane"
    worker_node_ips           = [
...
output "public_ips" {
  value = {
    "${local.CLUSTER_NAME}_control_plane_public" = aws_instance.control_plane[0].public_ip
...
output "private_ips" {
  value = {
    "${local.CLUSTER_NAME}_control_plane_private" = aws_instance.control_plane[0].private_ip
